...
export default function MyRentalModal({ tag, request_id, setShowModal, setTag } : { tag: string, request_id: number, setShowModal: Dispatch<SetStateAction<boolean>>, setTag: Dispatch<SetStateAction<string>> }) {
                ...
                <form className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                    <div className="bg-white pb-2 px-2">
                        <div className="mt-5 text-center">
                            <div className="text-black mb-4 font-bold">Add the boardgame image</div>
                            <input name="image" type="file" accept="image/*"className="file-input file-input-primary"
                                onChange={async (e) => {
                    ...
                    <div className="bg-gray-50 px-4 py-3 flex flex-row justify-center gap-6 mb-2">
                        ...
                        <button type="button" className="inline-flex w-auto btn bg-gs_purple_gradient hover:bg-opacity-60 border-none min-h-9 h-9 px-7" onClick={() => handleSubmit()}>Confirm</button>
                    </div>
                </form>
                </div>
            </div>
