...
export default function RentingShippingList() {
  const [reservedRequests, setReservedRequests] = useState<
    RentingRequestJoinBoardgameJoinCustomer[]
  >([]);
  const [rentingRequests, setRentingRequests] = useState<
    RentingRequestJoinBoardgameJoinCustomer[]
  >([]);
    ...
    <div className="w-full md:w-10/12 h-[calc(100vh-240px)] md:h-[calc(100vh-180px)]">
      ...
      <div className="p-2 bg-white/10 rounded-lg lg:hidden">
        <div className="flex w-full gap-2 rounded-lg bg-white/10 p-2 mb-2 font-bold">
          <button
            className={`flex gap-2 items-center justify-center w-1/2 py-2 rounded-lg ${showReserved ? "bg-gs_purple_gradient" : "bg-transparent"}`}
        ...
        <div
          className={`flex w-full justify-center h-[calc(100vh-240px)] ${showReserved ? "" : "hidden"}`}
        >
          <RentingShippingCard
            title="shipped"
            icon={<PiPackage strokeWidth={5} />}
